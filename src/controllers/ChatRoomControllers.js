...
class ChatRoomControllers {
  ...
  async createRoomChat(usernameCreator, usernameParticipant) {
    ...
    const query = {
      text: 'INSERT INTO chat_room VALUES ($1, $2) RETURNING id',
      values: [id, usernameCreator],
    };
    ...
    const queryToParticipant = {
      ...
      values: [result.rows[0].id, usernameParticipant],
    };
    const roomId = await this._pool.query(queryToParticipant);
    if (!roomId.rowCount) {
